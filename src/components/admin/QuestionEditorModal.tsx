...
export function QuestionEditorModal({ isOpen, question, onSave, onCancel }: QuestionEditorModalProps) {
  ...
  useEffect(() => {
    if (question) {
      ...
      if (question.assetKeys && question.format === 'Image Choice') {
        ...
        setImageAssets(assetKeys.map(key => ({key, url: key})));
      }
    } else {
  ...
  const loadAvailableImages = async () => {
    ...
    try {
      ...
      setImageError('Image API server not available. To enable image browsing, start the API server with: npm run start-api');
    } finally {
  ...
  const updateImageAsset = (index: number, key: string) => {
    const newAssets = [...imageAssets];
    newAssets[index] = { key: key, url: key };
    setImageAssets(newAssets);
  };
  ...
  const handleSliderArchetypeChange = (side: 'left' | 'right', archetype: ArchetypeName | '') => {
    setSliderArchetypes(prev => ({
      ...prev,
      [side]: archetype
    }));
  };
  ...
  const validateForm = () => {
    ...
    if (formData.format === 'Image Choice') {
      if (imageAssets.length === 0) {
        ...
        const hasEmptyKeys = imageAssets.some(asset => !asset.key.trim());
        if (hasEmptyKeys) {
          newErrors.images = 'All images must have valid image paths';
        }
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
   ...
   console.log('✅ Validation passed');
    ...
    try {
      // Process image paths for Image Choice questions
      let assetKeysString = '';
      if (formData.format === 'Image Choice' && imageAssets.length > 0) {
        const imagePaths = imageAssets
          .filter(asset => asset.key.trim())
          .map(asset => asset.key.trim());
        assetKeysString = `img:${imagePaths.join(',')}`;
      }
     ...
     console.log('🗺️ Generated parsedMapping:', parsedMapping);
      ...
      const updatedQuestion: ParsedQuestion = {
        ...
        assetKeys: formData.format === 'Image Choice' ? assetKeysString : undefined,
      ...
      if (formData.format === 'Image Choice') {
       console.log('🖼️ Validating Image Choice question...');
        if (!updatedQuestion.assetKeys || updatedQuestion.assetKeys === 'img:') {
         console.log('❌ Image paths validation failed:', updatedQuestion.assetKeys);
          throw new Error('Image Choice questions must have valid image paths');
        }
      ...
      {formData.format === 'Slider' ? (
        ...
        <div className="space-y-6">
          <div className="bg-purple-50 p-4 rounded-lg">
            ...
            <p className="text-sm text-purple-800">
              Assign percentage weights to archetypes for each image option. Weights don't need to total 100%. This step is optional and can be completed later.
            </p>
          </div>
          ...
          {imageAssets.length === 0 || imageAssets.every(asset => !asset.key.trim()) ? (
            <div className="text-center py-8 bg-gray-50 rounded-lg">
              <ImageIcon className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <h3 className="text-lg font-medium text-gray-900 mb-2">No Image Paths Added</h3>
              <p className="text-gray-600">
                Go back to the Images tab to add image paths before setting up archetype mappings.
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              {imageAssets.filter(asset => asset.key.trim()).map((asset, assetIndex) => (
